...
public class ChatroomQueryServiceImpl {
    ...
    public Slice<ChatroomResponse> getChatrooms(Long userId, Pageable pageable) {
        ...
        List<ChatParticipatingUser> getChatroomsAndUsers = chatParticipatingUserQueryService.getChatParticipatingUsers(user);
        // 3. 현재 유저가 포함 되어있는 복합키는 제외한다.
        List<ChatParticipatingUser> chatroomAndUsers = getChatroomsAndUsers.stream()
        ...
        chatroomAndUsers
                .forEach(chatParticipatingUser -> {
                    ...
                    Duration time = Duration.between(LocalDateTime.now(), chat.getCreatedAt());
                    // 2. chatQueryService.findByChatroomId...(chatroomId) 읽지 않은 채팅 개수
